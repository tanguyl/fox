...
ERL_NIF_TERM term_to_array(ErlNifEnv* env, ERL_NIF_TERM term, array* dest){
    ErlNifBinary* bin;
    if(!enif_inspect_binary(env, term, bin))
        return exception("Could not read term into a binary");
    
    int* read_i = (int*) bin.data;
    dest->size     = read_i[0];
    dest->n_dims   = read_i[1]; 
    dest->shape    = read_i + 2;
    dest->stride   = read_i + 2 + a.n_dims;
    dest->content  = (double*) (a.stride+a.n_dims);
