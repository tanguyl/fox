...
ERL_NIF_TERM alloc_set_broadcast_iterator(ErlNifEnv* env, int argc, ERL_NIF_TERM* argv, array_it* it){
    ERL_NIF_TERM error = argc==2? 0:exception(env, "Not enough arguments.");
    if( error 
            || (error = term_to_array(env, argv[0], &it->lhs))
            || (error = term_to_array(env,argv[1], &it->rhs)))
        return error;
